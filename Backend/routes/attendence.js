...
router.post('/attend', fetchuser, attendance_upload.single("file"),async (req,res)=>{
    ...
    try {
        ...
        let temp_photourl=`http://localhost:5001/api/a/newupload/${rfile}`
        let user_photourl=`http://localhost:5001/api/a/newupload/${usera.photo_url}`
...
const newve = router.post("/newupload", upload.single("file"), fetchuser,async (req, res) => {
    ...
    const imgUrl = `http://localhost:5001/api/a/newupload/${req.file.filename}`;
